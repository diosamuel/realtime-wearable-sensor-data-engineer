...
def _activity_label_expr():
    mapping_items = []
    for activity_id, activity_label in SELECTED_ACTIVITIES.items():
        mapping_items.extend([F.lit(activity_id), F.lit(activity_label)])
    return F.create_map(*mapping_items)


def _activity_category_expr():
    mapping_items = []
    for activity_label, activity_category in ACTIVITY_CATEGORIES.items():
        mapping_items.extend([F.lit(activity_label), F.lit(activity_category)])
    return F.create_map(*mapping_items)


def _dataset_file_paths(data_path, selected_activities):
    file_paths = []
...
def loadAllData(spark, data_path, selected_activities, sensor_columns):
    ...
    file_paths = _dataset_file_paths(data_path, selected_activities)
    path_col = F.input_file_name()
    activity_label_map = _activity_label_expr()
    ...
    return (
        ...
        .schema(schema)
        .csv(file_paths)
        .withColumn('source_file', path_col)
...
def prepareFeatureDatasets(spark, write_parquet=True):
    raw_df = loadAllData(spark, DATA_PATH, SELECTED_ACTIVITIES, SENSOR_COLUMNS)
    raw_df, mag_columns = addMagnitudeColumns(raw_df)
    all_columns = SENSOR_COLUMNS + mag_columns

    feature_df = (
        extractFeaturesPerSegment(raw_df, all_columns)
        .withColumn('activity_category', _activity_category_expr()[F.col('activity_label')])
    )
    ...
    train_df = feature_df.filter(F.col('person_id').isin(TRAIN_PERSONS))
    test_df = feature_df.filter(F.col('person_id').isin(TEST_PERSONS))

    if write_parquet:
        os.makedirs(SAVE_DIR, exist_ok=True)
        raw_df.write.mode('overwrite').parquet(RAW_PARQUET_PATH)
        feature_df.write.mode('overwrite').parquet(FEATURE_PARQUET_PATH)
        train_df.write.mode('overwrite').parquet(TRAIN_PARQUET_PATH)
        test_df.write.mode('overwrite').parquet(TEST_PARQUET_PATH)
    ...
    return {
        ...
        'feature_path': FEATURE_PARQUET_PATH,
